...
class LookupTable(object):
    ...
    * 7-5-4-3-2 unsuited (worst hand possible)  => 7462
    ...
    def flushes(self):
        ...
        rank = 1
        for sf in straight_flushes:
            prime_product = self.card.prime_product_from_rankbits(sf)
        ...
        for f in flushes:
            prime_product = self.card.prime_product_from_rankbits(f)
            self.flush_lookup[prime_product] = rank
            rank += 1
    ...
    def straight_and_highcards(self, straights, highcards):
        ...
        for s in straights:
            prime_product = self.card.prime_product_from_rankbits(s)
            self.unsuited_lookup[prime_product] = rank
            rank += 1

        rank = LookupTable.MAX_PAIR + 1
        for h in highcards:
            prime_product = self.card.prime_product_from_rankbits(h)
            self.unsuited_lookup[prime_product] = rank
            rank += 1
    ...
    def multiples(self):
        ...
        for i in backwards_ranks:
            ...
            kickers.remove(i)
            for k in kickers:
                product = self.card.PRIMES[i] ** 4 * self.card.PRIMES[k]
        ...
        for i in backwards_ranks:
            ...
            pairranks.remove(i)
            for pr in pairranks:
                product = self.card.PRIMES[i] ** 3 * self.card.PRIMES[pr] ** 2
                self.unsuited_lookup[product] = rank
                rank += 1
        ...
        for r in backwards_ranks:
            ...
            for kickers in gen:
                c1, c2 = kickers
                product = self.card.PRIMES[r] ** 3 * self.card.PRIMES[c1] * self.card.PRIMES[c2]
                self.unsuited_lookup[product] = rank
                rank += 1
        ...
        for tp in tpgen:
            ...
            for kicker in kickers:
                product = self.card.PRIMES[pair1] ** 2 * self.card.PRIMES[pair2] ** 2 * self.card.PRIMES[kicker]
                self.unsuited_lookup[product] = rank
                rank += 1
        ...
        for pairrank in backwards_ranks:
            ...
            for kickers in kgen:
                k1, k2, k3 = kickers
                product = self.card.PRIMES[pairrank] ** 2 * self.card.PRIMES[k1] \
                          * self.card.PRIMES[k2] * self.card.PRIMES[k3]
                self.unsuited_lookup[product] = rank
                rank += 1
