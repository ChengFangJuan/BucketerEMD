...
from Holdem.getcard import GetCard
from Holdem.deuces.evaluator import Evaluator
from Holdem.gamestate import GameState
...
class Table:
    ...
    def _dealHoleCards(self):
        ...
        for p in self._playing:
            ...
            if self._vocal: print(p.getName() + '(' + str(p.getStack()) + ')', 'dealt', self._deck[0], 'and',
                                  self._deck[1])
    ...
    def _payWinners(self):
        ...
        for p in self._playing:
            if not board:
                ...
                rank = self._eval.evaluate(board, [p.show()[0].toInt(), p.show()[1].toInt()])
            ranks[p] = rank
        ...
        while sum(self._s.bets) > 0:  # to handle n side pots
            ...
            for i in range(self._s.numP):
                if not i in self._s.folded and self._s.bets[
                    ...
                    if minLiveBet == None:
                        minLiveBet = self._s.bets[i]
                    else:
                        minLiveBet = min(minLiveBet, self._s.bets[i])
                    player = self._playing[i]
                    eligibleWinners.append(player)
                    if minRank == None:
                        minRank = ranks[player]
                    else:
                        minRank = min(minRank, ranks[player])
        ...
        if action[0] == 'check':
            ...
            if self._vocal:
                if not allIn:
                    ...
                    print(player.getName(), 'raises all-in', raiseBy, 'to', raiseTo)
        else:
