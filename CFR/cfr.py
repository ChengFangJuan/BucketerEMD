...
class Game():
    ...
    # 发牌
    def deal_cards(self):
        sample = random.sample(self.DECK, 4)
        player_one_cards = sample[0:2]
        player_two_cards = sample[2:]
        return player_one_cards, player_two_cards

    def deal_cards_biased(self, player_one_favored):
        rand_hand = random.sample(self.DECK, 2)
        strong_hand = list()
        # Strong suited ace
...
class CFR():
    ...
    def simplify_hand(self, hand):
        ...
        hand = self.Game.get_higher_rank(rank1, rank2)
    ...
    def get_winner(self, hand1, hand2):
        ...
        if is_hand1_pair and is_hand2_pair:
            if hand1[0] == hand2[0]:
                return 0
            if hand1[0] == self.Game.get_higher_rank(hand1[0],hand2[0]):
                return 1
            else:
                return 2
        elif is_hand1_pair:
        ...
        if is_hand1_suited and is_hand2_suited:
            if hand1[0] == hand2[0]:
                if hand1[1] == hand2[1]:
                    return 0
                if hand1[1] == self.Game.get_higher_rank(hand1[1], hand2[1]):
                    return 1
                return 2
            if hand1[0] == self.Game.get_higher_rank(hand1[0],hand2[0]):
                return 1
    ...
    def train(self, iterations, ante=1.0, bet1=2.0, bet2=8.0, print_interval=1000000):
        ...
        for i in range(iterations):
            ...
            player_one_cards, player_two_cards = self.Game.deal_cards()
    ...
    def get_strategy(self):
        ...
        for state, node in self.game_states_.items():
            ...
            # player 1
            if len(history) == 0:
                result[p1_bet][hand] = node.strategy_[self.Game.BET]
            # player 2
            elif len(history) == 1:
                if history[0] == self.Game.CHECK:
                    result[p2_bet][hand] = node.strategy_[self.Game.BET]
                else:
                    result[p2_raise][hand] = node.strategy_[self.Game.BET]
                    result[p2_call][hand] = node.strategy_[self.Game.CALL]
            #player 1
            elif len(history) == 2:
                if history[0] == self.Game.BET:
                    result[p1_bet_call][hand] = node.strategy_[self.Game.CALL]
                else:
                    result[p1_check_raise][hand] = node.strategy_[self.Game.BET]
                    result[p1_check_call][hand] = node.strategy_[self.Game.CALL]
            #player 2
            elif len(history) == 3:
                result[p2_bet_call][hand] = node.strategy_[self.Game.CALL]
